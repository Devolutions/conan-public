...
class TestPackageConan(ConanFile):
    ...
    def test(self):
        ...
        for lib in libs:
            file_path = os.path.join(self.deps_cpp_info['freevnc'].rootpath, 'lib', lib)
        ...
        include_dir = os.path.join(self.deps_cpp_info['freevnc'].rootpath, 'include', 'freevnc')
        self.output.info('- should have at least 21 headers under %s' % include_dir)
        headers = glob.glob('%s/*.h' % include_dir)
        assert len(headers) >= 21, 'Number of headers: %s' % len(headers)
