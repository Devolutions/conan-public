...
class EmbeddedTerminalConan(ConanFile):
    ...
    def validate(self):
        if self.settings.os != "Linux":
            raise ConanInvalidConfiguration("embedded-terminal is only supported for Linux targets")
        if not self.options.shared:
            raise ConanInvalidConfiguration("embedded-terminal builds a shared wrapper library")
